...
同一 session 的一轮修改不会逐文件阻塞。session idle 后 epoch 才关闭并保留在审查列表中；未接受的 epoch 不会阻止后续工具或文件修改。无需把 `permission.edit` 设置为 `ask`。

## Shell 与冲突

文件工具 `edit/write/patch/apply_patch/multiedit` 会自动精确捕获。companion 会给 Bash/Shell 工具增加可选的 `writes: string[]` 参数，OpenCode 可直接声明会创建、修改、移动或删除的准确路径；参数在内置 shell 执行前会被移除。

旧版 OpenCode 未暴露可扩展 JSON Schema 时，继续支持首行标记：
...
`git merge/rebase/cherry-pick/revert/pull/am/commit/checkout/switch/reset/restore/stash` 会自动记录命令前后的 tracked dirty 路径和 commit tree 变化；发生冲突时保存 `base / ours / theirs`，可从冲突文件菜单分别与当前工作区比较。会删除未跟踪文件的 `git clean`、`stash -u/--all` 和 `checkout -f` 建议声明准确写路径，否则默认记录 coverage gap。

缺失或无效的 `writes` 不会阻止命令。未知 shell/custom tool 默认生成 coverage gap，详情在审查树中默认折叠，接受 epoch 前按批次显式确认。将 `ocReview.shellPolicy` 设为 `strict` 可直接阻止未知工具。
